...
const PressCard: React.FC<PressCardProps> = ({ item, index, layout = 'grid' }) => {
  ...
  const getCategoryColor = (category: string) => {
    switch (category) {
      case 'newspaper': return 'bg-blue-500 text-white';
      case 'magazine': return 'bg-green-500 text-white';
      case 'digital': return 'bg-purple-500 text-white';
      case 'award': return 'bg-orange-500 text-white';
        ...
        <Card className="group cursor-pointer hover:shadow-xl transition-all duration-300 transform hover:scale-[1.02] bg-white border border-gray-200 overflow-hidden">
          <CardContent className="p-0">
            <div 
              ...
              style={{ height: layout === 'masonry' ? `${cardHeight}px` : '240px' }}
            >
              {!imageError ? (
                <>
                  <Image
              ...
              <div className="absolute top-3 left-3">
                <Badge className={`${getCategoryColor(item.category)} font-semibold shadow-lg`}>
...
const Press: React.FC = () => {
  ...
  return (
    <section className="py-16 px-4 md:px-6 lg:px-8 bg-gradient-to-br from-gray-50 to-blue-50" id='press-release'>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            Explore our media coverage, press releases, and recognition in newspapers, 
            magazines, and digital platforms showcasing our impact and achievements.
          </p>
        </div>
              ...
              <TabsContent key={category.key} value={category.key} className="mt-0">
                ...
                <div className="text-center mb-8">
                  <p className="text-gray-600">
                    Showing <span className="font-semibold text-orange-600">{filteredPress.length}</span> articles
                    {activeCategory !== 'all' && (
                      <span> in <span className="font-semibold">{category.label}</span></span>
                    )}
                  </p>
                </div>
                ...
                {filteredPress.length === 0 && (
                  <div className="text-center py-16">
                    ...
                    <p className="text-gray-600">
                      No press coverage available in the {category.label} category.
                    </p>
                  </div>
                )}
              </TabsContent>
