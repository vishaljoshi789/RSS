...
const useAxios = (): AxiosInstance => {
  const { refreshToken, logout, initialized, isAuthenticated } = useAuth();
  ...
  const handleResponseError = useCallback(async (error: AxiosError) => {
    ...
    switch (error.response?.status) {
      case 400:
        apiError.message = (error.response.data as any)?.message || 'Bad request';
